...
@pytest.fixture
def cityscapes_dataset() -> Group:
    """Get the test dataset cityscapes generated by chameleon transformer."""
    ds_url = (
        "gs://mx-labs-integration-test-data/mxlabs-chameleon/chameleon-debug-data/cityscapes-debug/"
        "test_data_downscaled_cityscape_via_transformer_1.2.zarr"
    )
    return get_group(ds_url, "r")


@pytest.fixture
def cityscapes_dataset_optimized() -> Group:
    """The same as the original cityscapes sample data from chameleon except one more cache file generated by
    squirrel.
    """
    ds_url = (
        "gs://mx-labs-squirrel-git-test/test-data/test_data_downscaled_cityscape_via_transformer_1.2_optimized.zarr"
    )
    return get_group(ds_url, "r")
